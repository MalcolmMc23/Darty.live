...
export function findMatch(userId: string): string | null {
  ...
  // Remove stale users and make sure we filter out the current user
  const freshUsers = waitingUsers.filter(
    user => (now - user.timestamp < staleThreshold) && (user.id !== userId)
  );
  
  console.log(`Found ${freshUsers.length} potential matches`);
  ...
  if (existingIndex === -1) {
    ...
    console.log(`User ${userId} already in queue at position ${existingIndex}`);
  }
